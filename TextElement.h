...
#include "font_data.h"
enum class TextAnimation { None, Ellipsis };
...
class TextElement : public RenderElement {
 ...
 public:
  TextElement();
  TextElement(const std::string& text, int x, int y,
              unsigned int fontSize = 24);
  TextElement(const std::string& text, int x, int y, unsigned int fontSize,
              sf::Color color);
